...
class a_MerweScaledSigmaPoints(object):
    """
    ...
    def __init__(self, n, alpha, beta, kappa, sqrt_method=None, subtract=None):
        ...
        if sqrt_method is None:
            self.sqrt = a_np.linalg.cholesky
        else:
            self.sqrt = sqrt_method

        if subtract is None:
            self.subtract = a_np.subtract
        else:
    ...
    def sigma_points(self, x, P):
        ...
        try:
            ...
            print(lambda_ + n)
            dimension = P.shape[0]
            U = self.sqrt((lambda_ + n) * P + 0.001 * a_np.eye(dimension))
            return

        sigmas = x.reshape([1, -1])
        for k in range(n):
            next_row = self.subtract(x, -U[k]).reshape([1, -1])
            sigmas = a_np.concatenate((sigmas, next_row), axis=0)
        for k in range(n):
            next_row = self.subtract(x, U[k]).reshape([1, -1])
            sigmas = a_np.concatenate((sigmas, next_row), axis=0)
    ...
    def _compute_weights(self):
        ...
        c = .5 / (n + lambda_)
        self.Wc = a_np.full(2 * n + 1, c)
        self.Wm = a_np.full(2 * n + 1, c)
